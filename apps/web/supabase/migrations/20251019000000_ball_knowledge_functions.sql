...
BEGIN
	...
	filtered_results AS (
		SELECT *
		FROM related_ball_knowledge
		WHERE p_search IS NULL
			OR note ILIKE v_search_pattern
			OR source_type ILIKE v_search_pattern
			OR internal_notes ILIKE v_search_pattern
			OR about_coach_name ILIKE v_search_pattern
			OR about_university_name ILIKE v_search_pattern
	),
	counted_results AS (
		SELECT
			*,
	...
	ORDER BY
		CASE WHEN p_sort_direction = 'ASC' THEN
			CASE p_sort_column
				...
				WHEN 'review_after' THEN review_after::TEXT
...
-- GRANT PERMISSIONS
-- ============================================================================
GRANT EXECUTE ON FUNCTION get_ball_knowledge_for_coach TO authenticated;
GRANT EXECUTE ON FUNCTION get_ball_knowledge_for_university TO authenticated;
GRANT EXECUTE ON FUNCTION get_ball_knowledge_for_program TO authenticated;
