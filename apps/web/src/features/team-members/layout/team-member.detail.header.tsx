...
export default function TeamMemberDetailHeader({
	teamMemberId,
	...
	const handleDeleteTeamMember = () => {
		executeDeleteTeamMember({ id: teamMemberId });
	};

	const fullName =
		teamMember?.first_name && teamMember?.last_name
			? `${teamMember.first_name} ${teamMember.last_name}`.trim()
			: teamMember?.first_name || teamMember?.last_name || "Team Member";
		...
		<div className="sticky top-0 z-10 flex h-[45px] flex-shrink-0 items-center justify-between border-border border-b px-4 py-2 lg:px-6">
			<div className="flex items-center gap-2">
				...
				<h1 className="font-medium text-[13px]">
					{fullName ? `${fullName} - Details` : "Team Member Details"}
				</h1>
			</div>
			<Dialog.Root
